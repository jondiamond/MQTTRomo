...
- (void)viewDidLoad
{
    ...
    if (!self.manager) {
        ...
        self.manager.subscriptions = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:MQTTQosLevelExactlyOnce]
                                                                 forKey:[NSString stringWithFormat:@"%@/%@/command/#",
...
- (void) receiveNotification:(NSNotification *) notification
{
    ...
    if ([[notification name] isEqualToString: @"RMPictureModuleDidTakePictureNotification"]) {
        NSDictionary *userInfo = notification.userInfo;
        
        UIImage * image = userInfo[@"photo"];

        NSData* photoData = UIImageJPEGRepresentation(image, 0.9);
        
        [self.manager sendData:photoData
                         topic:[NSString stringWithFormat:@"%@/%@/vision/photo/%@",
                                [self.defaults stringForKey:@"base"],
                                [self.defaults stringForKey:@"romoId"],
                                [notification name]
                                ]
        ...
        NSLog(@"Data sent to %@/%@/vision/photo/%@",
              [self.defaults stringForKey:@"base"],
              [self.defaults stringForKey:@"romoId"],
              [notification name]
              );
...
- (void)didDetectFace:(RMFace *)face
{
    ...
    // Only send a message on the first instance of face recognition
    if (!self.faceAlreadyDetected) {
        
        [self.manager sendData:[@"Face detected" dataUsingEncoding:NSUTF8StringEncoding]
                         topic:[NSString stringWithFormat:@"%@/%@/vision/debug/detectedFace",
                                [self.defaults stringForKey:@"base"],
                                [self.defaults stringForKey:@"romoId"]
                                ]
                           qos:MQTTQosLevelExactlyOnce
        ...
        NSLog(@"Data sent to %@/%@/vision/debug/detectedFace",
              [self.defaults stringForKey:@"base"],
              [self.defaults stringForKey:@"romoId"]
              );
        
        // Start timer to take a photo every 2 seconds
        timer = [NSTimer scheduledTimerWithTimeInterval:2.0f
                                                 target:self selector:@selector(takeFacePhoto:) userInfo:nil repeats:YES];
        
        self.faceAlreadyDetected = true;
    }
...
//    [[RMVisionDebugBroker shared] setDebugLabel:@"Attn: --" atIndex:1];
    ...
    if (self.faceAlreadyDetected) {
        ...
        [self.manager sendData:[@"Face detected" dataUsingEncoding:NSUTF8StringEncoding]
                         topic:[NSString stringWithFormat:@"%@/%@/vision/debug/lostFace",
                                [self.defaults stringForKey:@"base"],
                                [self.defaults stringForKey:@"romoId"]
                                ]
                           qos:MQTTQosLevelExactlyOnce
        ...
        NSLog(@"Data sent to %@/%@/vision/debug/lostFace",
              [self.defaults stringForKey:@"base"],
              [self.defaults stringForKey:@"romoId"]
              );
        
        // Remove the timer until a face is detected again
        [timer invalidate];
        timer = nil;
        
        self.faceAlreadyDetected = false;
    }
}

- (void) takeFacePhoto:(NSTimer *)timer
{
